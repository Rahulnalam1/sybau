...
export class JiraOAuthService {
  ...
  private async fetchWithAuth(url: string, options: RequestInit = {}) {
    ...
    console.log(`JiraService: Making ${options.method || 'GET'} request to: ${url}`);
    if (options.body) {
      console.log('JiraService: Request payload:', JSON.parse(options.body as string));
    }
    ...
    if (!response.ok) {
      ...
      console.error(`JiraService: API error (${response.status}):`, errorText);
  ...
  async getAccessibleResources() {
    try {
      const resources = await this.fetchWithAuth('https://api.atlassian.com/oauth/token/accessible-resources');
      return resources;
    } catch (error) {
  ...
  async getProjects(cloudId: string) {
    try {
      const projects = await this.fetchWithAuth(`https://api.atlassian.com/ex/jira/${cloudId}/rest/api/3/project`);
      return projects;
    } catch (error) {
  ...
  async getIssueTypes(cloudId: string) {
    try {
      const issueTypes = await this.fetchWithAuth(`https://api.atlassian.com/ex/jira/${cloudId}/rest/api/3/issuetype`);
      return issueTypes;
    } catch (error) {
  ...
  async getPriorities(cloudId: string) {
    try {
      const priorities = await this.fetchWithAuth(`https://api.atlassian.com/ex/jira/${cloudId}/rest/api/3/priority`);
      return priorities;
    } catch (error) {
  ...
  async createIssue(cloudId: string, params: {
    projectId: string;
    summary: string;
    description?: string;
    issueTypeId: string;
    ...
    try {
      console.log('JiraService: Creating issue with params:', { cloudId, ...params });
      
      const { projectId, summary, description, issueTypeId, assigneeId, priorityId } = params;
      ...
      if (priorityId && priorityId.trim() !== '') {
        console.log('JiraService: Including priority in request:', priorityId);
        try {
          issueData.fields.priority = {
            id: priorityId
          };
        } catch (prioError) {
          console.error('JiraService: Error adding priority (skipping):', prioError);
        }
      } else {
        console.log('JiraService: Skipping priority in request');
      }

      console.log('JiraService: Final issue data being sent to JIRA:', issueData);
  ...
  async createTestIssue(cloudId: string, projectId: string, issueTypeId: string) {
    console.log('JiraService: Creating test issue with:', { cloudId, projectId, issueTypeId });
    
    return this.createIssue(cloudId, {
      projectId,
      summary: "Test issue from Sybau",
      description: "This is a test issue created from the Sybau application.",
      issueTypeId
    });
  }
} 