...
export async function GET(request: Request) {
  try {
    ...
    if (!state || state !== 'random-state-value') {
      return NextResponse.redirect(new URL('/integrate/linear/error?error=invalid_state', request.url));
    }
    
    if (!code) {
      return NextResponse.redirect(new URL('/integrate/linear/error?error=no_code', request.url));
    }
    ...
    if (!tokenResponse.ok) {
      ...
      return NextResponse.redirect(new URL('/integrate/linear/error?error=token_exchange_failed', request.url));
    }
    ...
    const response = NextResponse.redirect(new URL('/integrate/linear/success', request.url));
    ...
    return NextResponse.redirect(new URL('/integrate/linear/error?error=server_error', request.url));
  }
} 